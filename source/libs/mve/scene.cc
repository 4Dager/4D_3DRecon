...
{
    std::cout << "Saving views to CGAII files..." << std::flush;
...
Scene::init_views (void)
{
    ...
    {
        if (views_dir[i].name.size() < 6)
            continue;
        if (util::string::right(views_dir[i].name, 6) != ".cgaii")
            continue;
    ...
    for (std::size_t i = 0; i < temp_list.size(); ++i)
    {
        ...
        {
            std::cout << "Warning loading CGAII file "
