...
{
    return "view_" + util::string::get_filled(id, 6) + ".cgaii";
}
...
import_bundle_nvm (AppSettings const& conf)
{
    ...
    std::cout << "Writing CGAII views..." << std::endl;
#pragma omp parallel for schedule(dynamic, 1)
    for (std::size_t i = 0; i < cameras.size(); ++i)
    {
        mve::CameraInfo& mve_cam = cameras[i];
        mve::NVMCameraInfo const& nvm_cam = nvm_cams[i];
        std::string fname = "view_" + util::string::get_filled(i, 4) + ".cgaii";
        ...
        /* Save view. */
#pragma omp critical
        std::cout << "Writing CGAII view: " << fname << "..." << std::endl;
...
find_max_scene_id (std::string const& view_path)
{
    ...
    {
        std::string ext4 = util::string::right(dir[i].name, 6);
        if (ext4 != ".cgaii")
            continue;
...
#pragma omp critical
        std::cout << "Importing image: " << fname
            << ", writing CGAII view: " << mve_fname << "..." << std::endl;
...
main (int argc, char** argv)
{
    ...
    util::system::print_build_timestamp("CGAII Makescene");
    ...
    args.set_description("This utility creates CGAII scenes by importing "
