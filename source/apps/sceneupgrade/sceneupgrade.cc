...
#define VIEWPORTS_SIGNATURE "CGAII_VIEWPORTS\n"
#define MATCHING_SIGNATURE_LEN 13
#define MATCHING_SIGNATURE "CGAII_MATCHING\n"
...
{
    std::string ext4 = util::string::lowercase(util::string::right(fname, 6));
	std::string _ext4 = util::string::lowercase (util::string::right (fname ,4));
    if (ext4 == ".cgaii")
...
convert_scene (AppSettings const& conf)
{
    ...
    for (std::size_t i = 0; i < dir.size(); ++i)
    {
        std::string ext4 = util::string::lowercase
            (util::string::right(dir[i].name, 6));
        if (ext4 != ".cgaii")
            continue;
...
main (int argc, char** argv)
{
    ...
    util::system::print_build_timestamp("CGAII Scene Upgrade");
    ...
    args.set_description("This utility upgrades an CGAII view, a prebundle "
        "file, or an CGAII scene to the new format. See the Github wiki for "
        "more details about the new formats. INPUT can either be a single "
        ".cgaii view, a single .sfm prebundle file, or a scene directory. In "
